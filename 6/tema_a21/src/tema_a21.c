...
//  print_args(argc, argv);
  ...
  if (argc - 1 != 3 && argc - 1 != 4) m_bac_err
  else if (argc - 1 == 3) {
    mode = 1;
  ...
  x = 0;
  while (x < 2) {
    pair[1 - x] = argv[argc - 1 - x][0];
    ++x;
  }

  if (mode == 0) {
    if (process_file(input_fd, output_fd, pair) < 0) m_pf_err
    if (close(input_fd) == -1) m_cl_err
    if (close(output_fd) == -1) m_cl_err
  }
  else if (mode == 1) {
    if (process_file(io_fd, io_fd, pair) < 0) m_pf_err
