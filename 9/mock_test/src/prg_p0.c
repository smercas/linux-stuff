...
int32_t main(int32_t argc, char** argv) {
  ...
  switch (pid1 = fork()) {
    ...
    case  0: {  //first child process
      ...
      if ((fd = open("fifo_P2toP1", O_RDONLY, 0600)) == -1) { perror("fifo_P2toP1 input open"); return -1; }
    ...
    default: {
      switch (pid2 = fork()) {
        ...
        case  0: {  //second child process
          ...
          if ((fd = open("fifo_P2toP1", O_WRONLY, 0600)) == -1) { perror("fifo_P2toP1 output open"); return -1; }
        ...
        default: {  //parent process
          ...
          if (close(p1[0]) == -1) { perror("p1[0] close"); return -1; }
          //map file
          if ((fd = open(argv[1], O_RDONLY, 0600)) == -1) { perror("input open"); return -1; }
          if (fstat(fd, &s) == -1) { perror("fstat"); return -1; }
          if (s.st_size == 0) { fprintf(stderr, "Recieved file (%s) is empty\n", argv[1]); return -1; }
          ...
          if (close(fd) == -1) { perror("input close"); return -1; }
          i = 0;
          j = 0;
          count = 0;
          while (i + j <= s.st_size) {
            if (i + j == s.st_size || strchr(" \n", input_map_address[i + j])) {
              i = i + j + 1;
              if (j != 0) {
                ++count;
                j = 0;
          ...
          if (count == 0) { fprintf(stderr, "No numbers need to be written\n"); return -1; }
          ...
          while (i + j <= s.st_size) {
            if (i + j == s.st_size || strchr(" \n", input_map_address[i + j])) {
              if (j != 0) {
                separator = input_map_address[i + j];
                ...
                // printf("number: %ld\n", number);
                if (write(p1[1], &number, sizeof(int64_t)) == -1) { fprintf(stderr, "numbers[%ld] write: ", i); perror(""); return -1; }
                input_map_address[i + j] = separator;
              }
              i = i + j + 1;
              if (j != 0) {
                j = 0;
          ...
          if (close(p1[1]) == -1) { perror("p1[1] close"); return -1; }
          if (munmap(input_map_address, s.st_size) == -1) { perror("input munmap"); return -1; }
          ...
          if (fstat(fd, &s) == -1) { perror("fstat"); return -1; }
          if (s.st_size == 0) { fprintf(stderr, "Recieved file (%s) does not have any numbers\n", recieved_file); return -1; }
          ...
          if (close(fd) == -1) { perror("output close"); return -1; }
          ...
          if (printf("max: %ld\nmin: %ld\n", max, min) < 0) { perror("max/min printf"); return -1; }
          if (munmap(input_map_address, s.st_size) == -1) { perror("output munmap"); return -1; }
